            ...
            <el-form ref="form" :model="form" :label-position="labelPosition">
                ...
                <h4 class="handletitle" style="margin-top:20px">
                    ...
                    <span style="float:right">
                        选择事件:
                        <el-select v-model="listvalue" placeholder="请选择" size="mini" style="width:90px;" @change="selectevent">
...
import { getModelTrainInfoById,getModelDteailByEvent,addModelInfo } from "./model_train.js"
...
export default {
    ...
    methods: {
        findData() {
            ...
            getModelTrainInfoById({modelId: modelId}).then(res => {
                // console.log(res.data);
                this.form = res.data.modelInfo;
                this.modelTotInfo = res.data.modelTotInfo; //模型综合评价
                this.eventInfo = res.data.eventInfo; //训练事件明细
                this.tableData.push(res.data);
                var option = res.data.eventList;
                ...
                // this.options = res.data.eventlist; //选择事件下拉框
            })
        },
        //选择事件
        selectevent() {
            let modelId = this.$route.query.id;
            getModelDteailByEvent({modelId: modelId,eventName: this.listvalue}).then(res => {
                // console.log(res.data);
